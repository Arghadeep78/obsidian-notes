...
- **Standard local variables** have automatic lifetime and block scope—they are born when execution enters their block and die when it exits.
- **Static local variables** overrides this they retain their block scope, but gain **program lifetime**, meaning they bypass the normal lifecycle to persist and hold their value across multiple function calls.
...
Scope: n C++, **scope** refers to ==the region of a program's text where a declared identifier (such as a variable, function, class, or enum) is visible and accessible==
...
Unlike the stack or heap, this memory allocated when program starts and only freed when program terminates. 
See [[00 - Core C++ Topics#Memory Layout — The Full Picture|Memory Layout]] for full more.
...
In C++, **high and low addresses** refer to the *two opposite ends* of a program's virtual memory space.
Different parts of your C++ program are mapped to specific regions *between* these two extremes.
...
int Student::count = 0; //<- **** outside .h containin class
int main() {
    ...
    cout << Student::count << "\n";   // 2 — access via class name (if not private)
    cout << s1.count << "\n";         // 2 — also valid, but class name is clearer
    cout << Student::getCount() << "\n";         // 2 — also valid access via static function (preferred)
}
```

*MANDATORY*: define (and optionally initialize) the static member OUTSIDE the class. This is where the actual memory is allocated for it.
*WHY* outside: The class definition lives in a header included by many .cpp files. If the static member were defined inside the class body, every translation unit that includes the header would produce its own definition → ODR (One Definition Rule) violation → linker error. Defining it once in a .cpp file guarantees exactly one memory location across the whole program.
- *Header (.h):* Declare the static member inside the class to let the compiler know it exists.
- *Source (.cpp):* Define the static member outside the class to allocate its single, shared memory location and avoid duplicate symbol errors.
...
A static member function belongs to the **class**, not to any individual object. You can call it without creating any object at all. (they can be called by object too obj.func())
...
all instances refer to the same objects
...
public:
    static Database& getInstance() {
        static Database db;   // created only once
        return db;
    }
    void connect() {
        cout << "Connected\n";
...
int main() {
    Database& d1 = Database::getInstance(); // creates the instance
    Database& d2 = Database::getInstance(); // returns same instance
    d1.connect();
    // d1 and d2 refer to the same object
}
```

## 0. Recap of Lecture 02 (as stated in video)
...
- The **ER (Entity-Relationship) Model is a data model**. Data models work at the **conceptual/logical layer** — the same layer where the DBA defines data: _what_ the data is, _what relationships_ exist among data, and _consistency constraints_.
- **Definition:** _"A high-level model based on perception of the real world that consists of a collection of basic objects called **entities** and **relationships** among those objects."_
...
- An entity is _a thing distinguishable from other things_. (A Student in a college is an entity; the College itself can be an entity.)
...
```
  ...
  Think of it like: Entity Set ≈ Class, Entity ≈ Object (in OOP)
```

> **Important (slang/usage):** While drawing ER diagrams, **"entity" and "entity set" are used interchangeably** — treat them the same when building the model.
...
- `Student`: Student ID, Name, Phone Number, Address, Batch, Courses, …
- `Customer`: Customer ID, Contact Number, Address details, DOB, Email, …

> **Design principle:** Keep attributes **limited and needed** — only those required to describe the entity (don't make the DB bulky; make it precise).

### Domain & Consistency Constraints

- Every attribute has a **domain** = the set of **permitted values** it can take.
- Examples: `Customer Name` cannot be numeric (no name is "Tooth 4"); `Loan` may have a domain {Car Loan, Home Loan, Education Loan} → a "Personal Loan" entry is not allowed.
- **Why constraints help:** they keep the database **consistent**. A badly designed DB without constraints could store a numeric name; on retrieval you'd get an illegal/inconsistent entry you can't properly define.
...
**Examples (named in video):**

- `Customer` **borrows** `Loan`
- `Customer` **deposits** / **withdraws** `Account`
- `Customer` **places** `Order` (e.g., on Amazon / food delivery)
...
> **ER Diagram acts as a BLUEPRINT for the DB.** When given a DB design task: (1) find **entities**, (2) find their **attributes**, (3) establish **relationships** → draw the full ER diagram. Later this blueprint is **converted into tables / the Relational Model** (covered in a later lecture).
...
- Cannot be further divided. Example: a customer's bank **account number** (a 10-digit number — taking the first/last 5 digits gives no separate meaning; used as a whole), a student's **roll number**.
...
- **Why divide:** lets you reference a part independently — e.g., fetch all customers whose **Last Name = Mukherjee**, or all citizens in a particular **PIN/ZIP code** (the same file-system problem from Lecture 01, now easy in a DB). Sub-parts are referable individually **and** together.
...
- Can have **multiple values**. Example: Phone Number (a customer can have several), Nominee Name, Dependent Name (multiple parents as dependents on a policy).
...
1. **Not Applicable** — the value genuinely doesn't apply. Example: a person with no middle name → middle name is NULL = not applicable (this is **not** an inconsistency).
2. **Unknown / Missing** — a value should exist but the entry is missing. Example: `Customer Name` is NULL even though the constraint says name must exist → the entry is missing (an inconsistency/problem).
3. **Not Known (yet)** — value exists but isn't known yet. Example: a new employee whose `Salary` isn't decided yet / onboarding in progress → salary NULL = not known yet.
...
- A weak entity has a **partial / discriminator key** (e.g., **Payment Number** — a sequential counter 1, 2, 3, …). A "Payment Number 1" only uniquely identifies a payment _within_ a particular loan (e.g., Car Loan's 1st installment).
- **Notation:** Weak entity → double rectangle. Weak relationship → double diamond. The partial/discriminator key of a weak entity → **dotted underline** (unlike the solid underline used for a primary key of a strong entity).
...
- One entity set related to itself. Example: `Employee` **manages** `Employee` (a manager is also an employee). _(Rare.)_
...
- Example: `Employee` **works-on** relating **Employee + Branch + Job**. An employee works on a branch; the employee has a job role; the job role exists in that branch → all three (Employee–Branch, Employee–Job, Job–Branch) are related in one relationship.
...
> Most relationships are **binary**; unary and ternary are uncommon — focus your thinking on binary relationships.
...
- Example: `Citizen has Aadhaar Card` (one citizen → one Aadhaar; a1↔b1, a2↔b2, …).
...
- Example: `Citizen has Car` (one citizen can own multiple cars b1, b2, b3; but each car is owned by exactly one citizen).
...
- This is simply the mirror of 1:N — it depends on which side you label A and which you label B.
- Example: `Course taught by Professor` — many courses (C1, C2) map to one professor (P1); i.e., one professor can teach many courses, but (in this view) a course maps to one professor.
...
- Examples: `Customer buys Product` (a customer buys many products; a product is bought by many customers); `Student attends Courses` (a student attends many courses; a course is attended by many students).
...
- Example: In `Customer borrows Loan`, the **Loan side has total participation** — every loan in the Loan entity set must be borrowed by some customer. There can be **no orphan ("lavaaris") loan** that no customer took.
...
> So: the **Customer side is partial** (a customer need not have a loan), but the **Loan side is total** (every loan must be borrowed by at least one customer — a "lavaaris" loan that no customer took cannot exist).
...
- **Strong entities may have total OR partial participation** (not necessarily total). In `Customer borrows Loan`, both are strong entities; Customer's participation is partial, Loan's is total.

---

## 13. Summary of ER Notations (as drawn in video)

```
  ER Diagram Notation Cheat Sheet (SDE-1 Interview Quick Reference)
...
- **Attribute** → ellipse; **Multi-valued** → double ellipse; **Derived** → dotted ellipse; **Primary Key** → solid underline; **Discriminator/Partial Key** (of a weak entity) → **dotted underline**.
- **Relationship** → diamond; **Weak Relationship** → double diamond.
- **Total Participation** → double lines.

---

> **Next lecture:** Extended ER Features (EER) — Generalization, Specialization, etc.; then hands-on practice drawing ER diagrams for real systems.
...
- A **table IS a relation** in the relational model. (Don't get confused — table and relation are the same thing.)
...
> ⚠️ Note the difference from ER model: in ER, _cardinality_ meant mapping cardinality; here **cardinality = number of tuples**.
...
**Customer table** (PK = Cust ID) | Cust ID | Name | Address | Contact Number |

**Order table** (PK = Order ID) | Order ID | Time Stamp (when order placed) | Delivery Date |
...
- **Columns = attributes.**
- **Relation Schema** — _defines the structure of the relation_: the relation name + its attributes. (e.g., the schema of Customer, the schema of Order.)
...
- Two entities (Customer, Order) with a relationship (`Places`). After converting both to tables, _how do we represent the `Places` relationship in tabular form?_ → use a **Foreign Key**.
...
|#|Constraint|Meaning|
|---|---|---|
|1|**NOT NULL**|Column must not accept NULL values. By default an attribute _can_ be NULL; `NOT NULL` enforces otherwise.|
